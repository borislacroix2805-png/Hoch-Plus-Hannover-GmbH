...
.toolbar{display:grid;grid-template-columns:1fr 1fr 1fr 1fr auto auto auto auto;gap:10px;margin-bottom:18px;align-items:center}
...
<p>Hoch Plus Hannover GmbH kann Arbeiten nach Gewerk und Kategorie pflegen und Änderungen nachvollziehbar speichern.</p>
</div></div>
...
return '<tr>'+
cell('Gewerk',tradeSelect(i,x.trade))+
...
return '<select onchange="items['+i+'].category=this.value;render()">'+categories.map(c=>'<option '+(c===v?'selected':'')+'>'+esc(c)+'</option>').join('')+'</select>';
}

function tradeSelect(i,v){
return '<select onchange="items['+i+'].trade=this.value;render()">'+trades.map(t=>'<option '+(t===v?'selected':'')+'>'+esc(t)+'</option>').join('')+'</select>';
}
...
if(oldVal!==newVal){
changes.push((aMap[id].work||bMap[id].work||id)+' · '+labelField(f)+': vorher "'+oldVal+'" → nachher "'+newVal+'"');
}
}
}

for(const id in bMap){
if(!aMap[id])changes.push('Gelöscht: '+(bMap[id].work||bMap[id].category||id));
}
...
if(!row){
return new Response(JSON.stringify({updatedAt:null,items:starter,categories:defaultCategories,trades:defaultTrades}),{status:200,headers});
}
...
trades:payload.trades||defaultTrades
}),{status:200,headers});
}
...
trades:body.trades||defaultTrades
});
...
).bind(crypto.randomUUID(),updatedAt,body.summary||'Änderung gespeichert',beforePayload,payload).run();
